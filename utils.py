...
def mkGroup(t):
    ...
    for p in c:
        s_p = []
        for s in p:
            s_p.append(standardSentenceProcessing(s))
        p_p.append(s_p)
    return p_p
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, paragraphs=None, batch_size=6, shuffle=True, x_size=3, embeddingLookup=None, stopChar=None, startChar="~"):
        ...
        if not paragraphs:  # FIXME: this will need to take the output of a list of TextLoader
    ...
    def __data_generation(self, paragraphs_temp):
        ...
        tokenized_para = list(
            map(lambda x: (list(dumbHigherMap(go, x))), paragraphs_temp))
        y = tokenized_para
        X = [(np.array(x)[np.random.choice(len(x), self.x_size).tolist()]).tolist()
             for x in y]  # Choose x_size sentences from each paragraph in y
        # concat all sentences in each paragraph
        y = [[self.startchar, ] + sum(x, []) for x in y]
